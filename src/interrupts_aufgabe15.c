...
__interrupt void ISR_Port1 (void) { 
	if(lpm_status==4) {	//Wenn wakeup durch Schalterdruck angefordert
		...
		tick=0;			//Tick zurucksetzen
		lpm_status=-1;	//Status Variable korrigieren
		WDTCTL = (WDTPW+WDTTMSEL+WDTCNTCL+WDTSSEL); //Watchog rauslassen
	}
	else {
		tick=0; //Wenn kein wakeup counter zurucksetzen
	}
...
__interrupt void ISR_Watchdog (void) {
	tick++; //tick hochsetzen
	P4OUT ^= 0x04; //Led toggeln
	if ((lpm_status == -1) && (tick > 60)) { //Wenn 60 sec im aktivemode fordere LPM4 an
		lpm_status=4;
		P4OUT |= 0x04; //.. und schalte gruene LED aus
	}
	CLEAR(IFG1, WDTIFG); // Clear all flags
//  ...hier den Code der ISR einfugen
}
//==============================================================
