...
void generate_array(ballstate *ball, barstate bar_left, barstate bar_right)
{
    ...
    valuesX[0] = (int)ball->x * RES_X/FIELD_X;
    valuesY[0] = (int)ball->y * RES_Y/FIELD_Y;
    
    count = ((ANTZ-1) / 2);
    for (i = 1; i < count + 1; i++) {
        valuesX[i] = 0;
        valuesY[i] = (bar_left - BARLENGTH/2.0) + (BARLENGTH / count) * i * RES_Y/FIELD_Y;
    }
    
    
    for (i = count + 1; i < ANTZ; i++) {
        valuesX[i] = RES_X;
        valuesY[i] = (bar_right - BARLENGTH/2.0) + (BARLENGTH / count) * (i-count) * RES_Y/FIELD_Y;
    }
}
//********************************************************************//
...
while(!stop) {
	...
	for(i=0;i<ANTZ;i++) {
		DrawPixel(pixels, valuesX[i]+1, valuesY[i], color);
		DrawPixel(pixels, valuesX[i], valuesY[i]+1, color);
		DrawPixel(pixels, valuesX[i], valuesY[i]-1, color);
		DrawPixel(pixels, valuesX[i]-1, valuesY[i], color);
		DrawPixel(pixels, valuesX[i], valuesY[i], color);
	}
	//****************************************************************//
